module Elasticsearch
  class Updater
    ...
    def self.get_first_data(teams)
      url = 'https://es02.firstinspires.org/teams_v1/_search?'
      search_args = {
        team_type: 'FTC',
        profile_year: (Season.where(offseason: false, active: true) || Season.where(offseason: false)).last.year.split('-')[0],
